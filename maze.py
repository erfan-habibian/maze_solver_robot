...
try:    
    with open('options.txt','r') as json_file:
        text = json_file.read()
    ...
    startx = int(input('enter the start position:'))
    starty = int(input())
    goalx = int(input('enter the goal position:'))
    goaly = int(input())
    start = (startx,starty)
    goal = (goalx,goaly)
...
rows = 30#data['rows']
columns = 30#data['columns']
start = (0,0)#(data['start'][0],data['start'][1])
goal = (29,29)#(data['goal'][0],data['goal'][1])
print('rows: ', rows)
